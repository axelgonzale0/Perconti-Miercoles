...
CONFIG_FILE = "servidores_guardados.json"

def cargar_servidores_guardados():
...
def guardar_servidor(nuevo):
    servidores = cargar_servidores_guardados()
    if nuevo not in servidores:
        servidores.append(nuevo)
...
def abrir_configuracion():
    ...
    ventana_config.title("Configurar Servidor")
    ventana_config.geometry("300x150")

    tk.Label(ventana_config, text="Ingresar IP o dominio:", font=("Arial", 11)).pack(pady=10)

    entrada_nueva = tk.Entry(ventana_config, font=("Arial", 11), width=25)
    entrada_nueva.pack(pady=5)

    def guardar():
        nuevo = entrada_nueva.get().strip()
        if nuevo == "":
            messagebox.showwarning("Error", "No puedes dejar vacío.")
            return
        
        guardar_servidor(nuevo)

        valores_actuales = list(combo_ping["values"])
        if nuevo not in valores_actuales:
            valores_actuales.append(nuevo)
            combo_ping["values"] = valores_actuales

        messagebox.showinfo("Guardado", "Servidor agregado correctamente.")
        ventana_config.destroy()

    tk.Button(ventana_config, text="Guardar", command=guardar).pack(pady=10)
...
servidores_base = ["google.com", "cloudflare.com", "facebook.com", "youtube.com", "8.8.8.8", "1.1.1.1"]
servidores_guardados = cargar_servidores_guardados()
todos_los_servidores = servidores_base + servidores_guardados

combo_ping = ttk.Combobox(
    frame_ping,
    values=todos_los_servidores,
    font=("Arial", 12),
    width=25
)
...
btn_configurar = tk.Button(frame_ping, text="Configurar", font=("Arial", 11), command=abrir_configuracion)
btn_configurar.grid(row=0, column=2, padx=6)
